/**
 * Digital Twin Runtime Landing Page & Auto 24h Cache Engine
 */
...
async function loadProjectData() {
  const LOCAL_CACHE_KEY = 'digitaltwin_run_github_daily_cache_v1';
...
function initCytoscapeGraph() {
  ...
  const elements = [];
  Object.values(appState.projects).forEach(p => {
    elements.push({ data: { id: p.id, label: p.id, color: '#f59e0b' } });
  });
  Object.values(appState.projects).forEach(p => {
